...
namespace WindowsFormsApp1
{
    ...
    {
        private List<Vertex> revealedPoints;       // visible points (clicked by user)
        private HashSet<Face> triangulation;       // current triangulation
        private Face superTriangle;                // initial super triangle
        private TriangulationBuilder triangulator;
        private MenuStrip menuStrip;               // menu bar
        private ToolStripMenuItem fileMenu;        // "File" menu
        private ToolStripMenuItem resetMenu;       // "Reset" option
        private Face hoveredFace = null;           // triangle under cursor
        private bool hasError = false;             // prevent further processing after critical error
        ...
        private void Form1_Paint(object sender, PaintEventArgs e)
        {
            ...
            try
            {
                ...
                using (Pen circlePen = new Pen(Color.Green, 1))
                {
                    ...
                    {
                        var pos = v.Position;
                        g.FillEllipse(pointBrush, pos.X - vertexRadius, pos.Y - vertexRadius, vertexRadius * 2, vertexRadius * 2);
                    }
